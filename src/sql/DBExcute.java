...
public class DBExcute {
	 ...
	 * @param Table
	 */
	public void UpdateDB(Context context, ContentValues values, String where,
			String[] args, String Table) {
		DBHelper dbHelper = new DBHelper(context);
		SQLiteDatabase db = dbHelper.getWritableDatabase();
	 ...
	 * @param Cust_id
	 */
	public void UpdateBDCustID(Context context, ContentValues values,
			String Title, String Cust_id) {
		DBHelper dbHelper = new DBHelper(context);
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		db.update(Constant.TB_Base, values, "Title=? and Cust_id=?",
				new String[] { Title, Cust_id });
		db.close();
		dbHelper.close();
	}
	 ...
	 * @param id
	 */
	public void DeleteDB(Context context, String table, String whereClause,
			String[] whereArgs) {
		DBHelper dbHelper = new DBHelper(context);
		SQLiteDatabase db = dbHelper.getWritableDatabase();
	 ...
	 * @param id
	 */
	public void updataVehilce(Context context, String tableName,
	...
	// 分页查询
	public List<AdressData> getPageDatas(Context context, String sql,
			String[] whereClause) {
	 ...
	 * 分页查询（车友圈）
	 */
	public List<Article> getArticlePageDatas(Context context, String sql,
			String[] whereClause, List<Article> articleData) {
		DBHelper dbHelper = new DBHelper(context);
	 ...
	 * 更新车友圈数据
	 */
	public void updateArticleComments(Context context, String tableName,
			int whereValue, String commentValue, String commentUser, int cust_id) {
		DBHelper dbHelper = new DBHelper(context);
		SQLiteDatabase reader = dbHelper.getReadableDatabase();
		SQLiteDatabase update = dbHelper.getWritableDatabase();
		Cursor cursor = reader.rawQuery("select * from " + tableName
				+ " where Blog_id=?",
				new String[] { String.valueOf(whereValue) });
		String content = "";
		String newContent = "";
		...
		try {
			...
			update.update(tableName, values, "Blog_id=?",
					new String[] { String.valueOf(whereValue) });
	...
	public void updateArticlePraises(Context context, String tableName,
			int whereValue, String praisesUser, int cust_id) {
		DBHelper dbHelper = new DBHelper(context);
		SQLiteDatabase reader = dbHelper.getReadableDatabase();
		SQLiteDatabase update = dbHelper.getWritableDatabase();
		Cursor cursor = reader.rawQuery("select * from " + tableName
	...
	public Article parseDBDatas(Cursor cursor) {
		...
		try {
			JSONObject jsonObject = new JSONObject(cursor.getString(cursor
					.getColumnIndex("Content")));
			article = new Article();
			article.setBlog_id(Integer.valueOf(cursor.getString(cursor
					.getColumnIndex("Blog_id"))));
			article.setUserLogo(cursor.getString(cursor
					.getColumnIndex("UserLogo")));
			...
			if (!"[]".equals(jsonObject.getString("comments"))) {
				...
				for (int m = 0; m < jsonArrayComment.length(); m++) {
					...
					JSONObject jsonObjectComment = jsonArrayComment
							.getJSONObject(m);
			...
			if (!"[]".equals(jsonObject.getString("pics"))
					&& !"[null]".equals(jsonObject.getString("pics"))) {
				...
				for (int j = 0; j < json.length(); j++) {
					...
					imageListTemp.put(Constant.smallImage,
							jsonObj.getString("small_pic"));
					imageListTemp.put(Constant.bigImage,
							jsonObj.getString("big_pic"));
			...
			if (!"[]".equals(jsonObject.getString("praises"))) {
				...
				for (int k = 0; k < json.length(); k++) {
					parisesList.put(json.getJSONObject(k).getString("cust_id"),
							json.getJSONObject(k).getString("name"));
				}
	...
	public String selectIllegal(Context context) {
		...
		Cursor cursor = reader.rawQuery("select * from "
				+ Constant.TB_IllegalCity, new String[] {});
		if (cursor.moveToFirst()) {
	 ...
	 * @param context
	 */
	public void updataComment(String whereValues, String updateTime,
			String comments, String praises, String TbName, Context context) {
		DBHelper dbHelper = new DBHelper(context);
		SQLiteDatabase reader = dbHelper.getReadableDatabase();
		SQLiteDatabase updateDatas = dbHelper.getWritableDatabase();
		Cursor cursor = reader.rawQuery("select * from " + TbName
				+ " where Blog_id=?",
				new String[] { String.valueOf(whereValues) });
		String content = "";
		String newContent = "";
		...
		try {
			...
			updateDatas.update(TbName, values, "Blog_id=?",
					new String[] { String.valueOf(whereValues) });
