...
public class CaptchaActivity extends Activity{
	...
	private void GetCaptcha(){
		...
		new Thread(new NetThread.GetDataThread(handler, url, get_captcha)).start();
	}
	...
	private void SubmitPhone(){
        ...
        if(!sp_pwd.equals(GetSystem.getM5DEndo(pwd))){
        	Toast.makeText(CaptchaActivity.this, "密码错误，请重新输入", Toast.LENGTH_SHORT).show();
        	return ;
        }
        if(!captcha.equals(valid_code)){
			Toast.makeText(CaptchaActivity.this, "验证码错误，请重新输入", Toast.LENGTH_SHORT).show();
			return ;
		}
	...
	private void jsonReset(String str){
		try {
			...
			if(jsonObject.getString("status_code").equals("0")){
				...
				Toast.makeText(CaptchaActivity.this, "修改失败", Toast.LENGTH_SHORT).show();
			}
		} catch (JSONException e) {
			e.printStackTrace();
		}
	...
	private void Submit(){
		...
		if(!pwd.equals(pwdAgain)){
			Toast.makeText(CaptchaActivity.this, "2次输入的密码不一致，请重新输入", Toast.LENGTH_SHORT).show();
			return ;
		}
		if(!captcha.equals(valid_code)){
			Toast.makeText(CaptchaActivity.this, "验证码错误，请重新输入", Toast.LENGTH_SHORT).show();
		...
		if(mark == 0 || mark == 2){
			Intent intent = new Intent(CaptchaActivity.this, RegisterInfoActivity.class);
			...
			//TODO 重置密码
			String url = Constant.BaseUrl + "customer/password/reset?account=" + account + "&password=" + GetSystem.getM5DEndo(pwd);
			List<NameValuePair> params = new ArrayList<NameValuePair>();
